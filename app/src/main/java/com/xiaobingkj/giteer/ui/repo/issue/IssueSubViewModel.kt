...
class IssueSubViewModel: BaseViewModel() {
    val errorEvent = MutableLiveData<AppException>()
    val issueEvent = MutableLiveData<List<IssueBean>>()

    fun getIssue(name: String, state: String, page: Int) {
        requestNoCheck({
            HttpRequestCoroutine.getIssue(name, state, page)
        }, {
