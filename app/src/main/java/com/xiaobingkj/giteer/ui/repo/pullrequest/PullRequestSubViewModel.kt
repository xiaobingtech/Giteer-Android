...
class PullRequestSubViewModel: BaseViewModel() {
    val errorEvent = MutableLiveData<AppException>()
    val issueEvent = MutableLiveData<List<PullRequestBean>>()

    fun getPulls(name: String, state: String, page: Int) {
        requestNoCheck({
            HttpRequestCoroutine.getPulls(name, state, page)
        }, {
            issueEvent.postValue(it)
        }, {
