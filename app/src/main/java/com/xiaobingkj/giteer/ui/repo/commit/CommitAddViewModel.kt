...
class CommitAddViewModel: BaseViewModel() {
    val commitEvent = MutableLiveData<CommitBean>()
    val errorEvent = MutableLiveData<AppException>()

    fun addCommit(name: String, path: String, content: String, sha: String, message: String, branch: String) {
        requestNoCheck({
            HttpRequestCoroutine.addCommit(name, path, content, sha, message, branch)
        }, {
